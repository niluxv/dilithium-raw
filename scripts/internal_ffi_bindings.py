...
def main():
    ...
    impl = "clean"
    IMPL = impl.upper()
    namespace_prefix = f"PQCLEAN_DILITHIUM{level}_{IMPL}_"

    def process_function_wrapping(rust: Rust, function: dict, wrapping_conf: dict, add_binding: bool = True):
        name = function["name"]
        c_name = namespace_prefix + name
...
"""
    ...
    for function in api["api"]:
        ...
        for wrapping_conf in wrappings:
            process_function_wrapping(rust_code, function, wrapping_conf, first)
            first = False

    rust_code.binding += f"}}\n"

    with open(f"src/internals/dilithium{level}/ffi.rs", 'w') as rust_file:
        rust_file.write("use super::*;\n")
        rust_file.write("use core::mem::MaybeUninit;\n")
        rust_file.write("use cty::c_int;\n")
        rust_file.write("\n")
