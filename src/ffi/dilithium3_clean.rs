...
mod tests {
    ...
    fn test_sign_verify() {
        ...
        let res = unsafe {
            PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_keypair(&mut pubkey as *mut u8, &mut seckey as *mut u8)
        };
        assert_eq!(res, 0);
        ...
        let mut len: usize = 0;
        let res = unsafe {
            PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_signature(&mut sig as *mut u8, &mut len as *mut usize, msg as *const u8, msg.len(), &seckey as *const u8)
        };
        assert_eq!(res, 0);

        let res = unsafe {
            PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_verify(&sig as *const u8, len, msg as *const u8, msg.len(), &pubkey as *const u8)
        };
