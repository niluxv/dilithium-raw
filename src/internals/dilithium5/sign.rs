//! Implementation of dilithium 5 clean `sign.c` ported to Rust.
...
pub fn crypto_sign_keypair(
    ...
    // 2* SEEDBYTES`
    let key =
        slice_as_array::<u8, SEEDBYTES>(&random[SEEDBYTES + CRHBYTES..2 * SEEDBYTES + CRHBYTES]);
...
pub fn crypto_sign_signature(
    ...
    loop {
        ...
        let sig_slice = slice_as_array_mut::<u8, { params::K * params::POLYW1_PACKEDBYTES }>(
